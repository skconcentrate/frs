...
vec_frs_bestrid=zeros(1,simulateno);
vec_ars_bestrid4all_af=zeros(1,simulateno);
vec_ars_bestrid4all_df=zeros(1,simulateno);
...
% %         vec_rweight(1,stano)=vec_rri_pw(1,stano)/sum(vec_rri_pw(1,:));
    ...
    %暂时选取最优的R作为唯一的R
    vec_frs_bestrid(caltimes)=vec_sortweight(1,ui_validrno);
...
for link_temp=1:ui_linkno
    ...
    %暂时选取最优的R作为唯一的R
    vec_ars_bestrid4all_af(caltimes)=vec_sortrsnr4all_af(1,ui_sumr);
    vec_ars_bestrid4all_df(caltimes)=vec_sortrsnr4all_df(1,ui_sumr);
    vec_ARS_AF_RESULT(1,caltimes)=vec_sortrsnr4all_af(4,ui_sumr);
    vec_ARS_DF_RESULT(1,caltimes)=vec_sortrsnr4all_df(5,ui_sumr);
    
    if (0 ~= vec_frs_bestrid(caltimes))
        if ((vec_sortrsnr4all_af(4,ui_sumr) < vec_rsnr4all(4,vec_frs_bestrid(caltimes)-2*ui_linkno)) || (vec_sortrsnr4all_df(5,ui_sumr) < vec_rsnr4all(5,vec_frs_bestrid(caltimes)-2*ui_linkno)))
            disp(sprintf('wrong no:af=%d\tdf=%d\tfrs=%d\t%d\n',vec_sortrsnr4all_af(4,ui_sumr),vec_sortrsnr4all_df(5,ui_sumr),vec_rsnr4all(4,vec_frs_bestrid(caltimes)-2*ui_linkno),vec_rsnr4all(5,vec_frs_bestrid(caltimes)-2*ui_linkno)));
        end
    end
...
%AF%
if (0 ~= vec_frs_bestrid(caltimes))
    vec_FRS_AF_RESULT(1,caltimes)=vec_rsnr4all(4,vec_frs_bestrid(caltimes)-2*ui_linkno);
end

%DF
if (0 ~= vec_frs_bestrid(caltimes))
    vec_FRS_DF_RESULT(1,caltimes)=vec_rsnr4all(5,vec_frs_bestrid(caltimes)-2*ui_linkno);
end
...
disp(sprintf('FRS:\t%d\t%d\n',ui_mean_frs_caf,ui_mean_frs_cdf));
disp(sprintf('ERS:\t%d\t%d\n',ui_mean_ars_caf,ui_mean_ars_cdf));
