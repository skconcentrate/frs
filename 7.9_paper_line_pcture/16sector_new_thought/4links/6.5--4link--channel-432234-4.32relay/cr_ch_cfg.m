...
%  *************************************************************************************/
%信道的配置，输入的参数为距离和波束宽度
function [cfg] = cr_ch_cfg(dis,beamwidth)

% COMMON PARAMETERS

cfg.Pnorm       = 1;        % normalization parameter: 0 - w/o normalization, 1 - apply normalization for output channel impulse response
cfg.sample_rate = 2.56;     % sample rate in [GHz] applied for continuous time to discrete time channel impulse response conversion

% CFG.CR SUBSTRUCTURE   
% 选择子场景
cfg.cr.ap_sp = 0;           % parameter selects subscenario: 0 - STA-STA subscenario, 1 - STA-AP subscenario

cfg.cr.D    = dis;          % distance in [meters] between TX and RX, note that when the AP is placed near the ceiling distance D between TX and RX is set in horizontal plane
cfg.cr.Plos = 1;            % LOS (Line-of-Sight) parameter, permitted values: 0 - corresponds to NLOS scenario, 1 - corresponds to LOS scenario
...
cfg.bf.bf_alg = 2;          % beamforming algorithm: 0 - max power ray algorithm, 1 - max power exhaustive search algorithm
...
cfg.bf.tx_ant_type = 1;     % TX antenna type, permitted values: 0 - isotropic radiator, 1 - basic steerable directional antenna
cfg.bf.rx_ant_type = 1;     % RX antenna type, permitted values: 0 - isotropic radiator, 1 - basic steerable directional antenna
...
cfg.bf.tx_hpbw = beamwidth;        % TX antenna beamwidth
cfg.bf.rx_hpbw = beamwidth;        % RX antenna beamwidth
